...
//then the validation check is to see if the refund period overlaps
  Feature("01 - Validate a duplicate draft claim for each EU member state - Error checking") {

    Scenario("01 - Validate a duplicate draft refund from the EU member state page", Error) {
      Given("I login as an organisation")
      //      Database
//      AuthorityWizard.login("Organisation", "999900002")
      //      Stub
      AuthorityWizard.login("Organisation", "111111111")
