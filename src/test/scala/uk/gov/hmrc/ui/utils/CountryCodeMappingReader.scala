...
/** Represents one parsed row from the CountryCodeMapping.xlsx spreadsheet.
  ...
  *
  * Labels are stored separately from codes so tests can assert the UI text shown to the user, not just the numeric
  * identifiers.
  */
...
/** Reads CountryCodeMapping.xlsx from the test resources and converts it into structured MappingRow objects that can be
  * used by data-driven UI tests.
  *
  * Expected spreadsheet shape: 0 = Country 1 = Country Code 2 = Code 3 = Sub Codes 4 = Sub Category Codes
  */
object CountryCodeMappingReader {

  /** Special literal used in the spreadsheet to represent a real selectable "None" option in the UI.
    */
  private val NoneValue = "None"

  /** Loads the workbook from the classpath.
    *
    * By default this reads: /CountryCodeMapping.xlsx
    *
    * @param resourcePath
    *   the classpath resource path
    * @return
    *   all parsed mapping rows from Sheet1
    */
  ...
  /** Loads the workbook from an InputStream and parses rows from Sheet1.
    *
    * The header row is skipped and every remaining row is converted to an optional MappingRow. Invalid or incomplete
    * rows are ignored.
    *
    * @param inputStream
    *   Excel file input stream
    * @return
    *   sequence of parsed MappingRow values
    */
  ...
  /** Parses a single Excel row into a MappingRow.
    ...
    *
    * Code column is mandatory. Sub code and sub-category code are optional.
    *
    * @param row
    *   Excel row
    * @return
    *   Some(MappingRow) if valid, otherwise None
    */
  ...
  /** Safely reads a cell as trimmed text.
    ...
    * @return
    *   trimmed cell text or empty string
    */
  ...
  /** Splits an optional spreadsheet field into:
    ...
    *   - "17" -> (Some("17"), Some("17"))
    *
    * This allows the reader to preserve "None" as a real selectable UI option, rather than treating it as missing data.
    *
    * @param raw
    *   raw cell text
    * @return
    *   optional code and optional label
    */
