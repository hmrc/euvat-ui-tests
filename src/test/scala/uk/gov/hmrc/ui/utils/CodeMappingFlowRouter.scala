...
/** Central routing helper for both Purchase and Import flows.
  ...
  *
  * Purchase URLs are under /purchase/... Import URLs are under /import/...
  */
object CodeMappingFlowRouter {

  /** Defines which journey is being tested. pathSegment controls the URL prefix. entrySlug is the first page slug for
    * that journey.
    */
  ...
  /** Builds a journey-specific page slug. Example:
  ...
  /** Special-case slug for the "Other" top-level page because purchases use purchase-type-other and imports use
    * import-type-other.
    */
  ...
  /** Special-case title for the "Other" top-level page because purchase and import journeys use different wording.
    */
  ...
  private def fuelTypeOrVehiclePage(flowType: FlowType) =
    new GenericRadioPage(
      slug(flowType, "fuel-type-or-vehicle"),
      "What is the type of fuel or vehicle? - EU VAT - GOV.UK"
    )
  ...
  private def whatTransportCostPage(flowType: FlowType) =
    new GenericRadioPage(
      slug(flowType, "what-transport-cost"),
      "What option best describes this transport cost? - EU VAT - GOV.UK"
    )
  ...
  private def publicityPage(flowType: FlowType) =
    new GenericRadioPage(
      slug(flowType, "cost-for-publicity-purposes"),
      "Is this cost for publicity purposes? - EU VAT - GOV.UK"
    )

  private def propertyPurchaseTypePage(flowType: FlowType) =
    new GenericRadioPage(
      slug(flowType, "property-purchase-type"),
      "What is the type of property purchase? - EU VAT - GOV.UK"
    )
  ...
  /** Returns the journey entry page slug. Example:
