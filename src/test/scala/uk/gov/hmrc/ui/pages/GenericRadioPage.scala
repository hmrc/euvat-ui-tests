...
/** Generic page object for GOV.UK-style radio button pages.
  ...
  *
  * It also includes retry handling for stale element exceptions, which can happen when the page re-renders during
...
class GenericRadioPage(
  ...
  /** CSS selector used to find the visible labels for GOV.UK radio items.
    *
    * Example HTML pattern: <div class="govuk-radios__item"> <input ... /> <label>Some option</label> </div>
    */
  ...
  /** Reads all visible radio labels from the current page once.
    ...
    *
    * It does not retry if the DOM changes mid-read, so callers that need resilience should use `availableLabels()`.
    */
  ...
  /** Returns all currently visible radio option labels on the page.
    ...
    *
    * This is useful because radio pages may refresh or re-render while Selenium is attempting to read the label text.
    */
  ...
  /** Selects a radio option by matching its visible label text.
    ...
    *   - clicks Continue
    *
    * It retries up to 5 times if the DOM changes and Selenium throws a StaleElementReferenceException.
    *
    * @param label
    *   the exact visible label text to select
    * @return
    *   this page object, to allow fluent chaining
    */
  ...
  /** Asserts that the radio labels on the page contain exactly the same elements as the expected values, regardless of
    * ordering.
    *
    * This is used by mapping tests to verify that the UI options match the expected labels from the spreadsheet.
    */
  def assertLabelsContain(expected: Seq[String]): Unit =
    availableLabels() must contain theSameElementsAs expected

  /** Verifies that the browser is currently on the expected page by checking the page title.
    *
    * This is useful after navigation or selection to confirm the flow has reached the correct radio page.
    */
